      ...
      if opts[:update_db]
        Sidekiq.redis do |redis|
          pools.each do |pool|
            ...
            pool.brackets.where.not(id: best_hash.keys).map(&:bracket_point).each { |bp| bp.update(best_possible: 60_000) }
            best_hash.each do |bracket_id, rank|
              Bracket.find(bracket_id).bracket_point.update(best_possible: rank.to_i)
            end
          end
        end
      end
