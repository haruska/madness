...
RSpec.describe Tournament, type: :model do
  ...
  describe '#round_for' do
    ...
    context 'round 1' do
      let(:expected_game_slots) do
        Team.where(starting_slot: (64...80).to_a).map(&:first_game).map(&:slot).uniq
      end

      it 'returns games for the region ordered by team seed (1,8,5,4,6,3,7,2)' do
        expect(subject.round_for(1, region).map(&:slot)).to eq(expected_game_slots)
      end
    end
