...
RSpec.describe Game, type: :model do
  ...
  describe 'calculating a value' do
    let(:tree) { TournamentTree.new }
    let(:nodes) do
      TournamentTree::SIZE.times.collect do |i|
        node = tree.at(i)
    ...
    context 'when decision is left' do
      before do
        nodes.each { |n| n.decision = Game::LEFT unless n.nil? }
      end
    ...
    describe 'when decision is right' do
      before do
        nodes.each { |n| n.decision = Game::RIGHT unless n.nil? }
      end
  ...
  describe 'a leaf node' do
    let(:node) { TournamentTree.new.at(63) }
  ...
  describe 'current depth calculation' do
    let(:depth_hash) do
      ...
      # bfs
      tree = TournamentTree.new # 63 nodes
      TournamentTree::DEPTH.times do |i|
        current_depth = i + 1
        depth_hash[current_depth] = []
        if current_depth == 1
          depth_hash[current_depth] << tree.championship
        else
  ...
  describe '#team_one' do
    context 'a leaf node' do
      ...
      it 'is the team with the left_position starting slot' do
        expect(subject.team_one).to eq(Team.find_by(starting_slot: subject.left_position))
      end
    end
  ...
  describe '#team_two' do
    context 'a leaf node' do
      ...
      it 'is the team with the right_position starting slot' do
        expect(subject.team_two).to eq(Team.find_by(starting_slot: subject.right_position))
      end
    end
  ...
  describe '#team' do
    ...
    it 'is the team with the current starting slot' do
      expect(subject.team).to eq(Team.find_by(starting_slot: subject.value))
    end

    it 'is aliased to winner' do
      expect(subject.winner).to eq(Team.find_by(starting_slot: subject.value))
    end
  end
  ...
  #   let(:bracket) { create(:bracket, :completed, pool:) }
